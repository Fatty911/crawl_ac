...
try:
    ...
    from merge_data import check_publication, normalize_model_identity


def identities(payload: dict[str, Any], eligible_only: bool = False) -> set[str]:
    result = set()
    for item in payload.get("items", []):
        if eligible_only and not check_publication(item)[0]:
            continue
        ...
        result.add(normalize_model_identity(item.get('model') or item.get('title')))
    return result
